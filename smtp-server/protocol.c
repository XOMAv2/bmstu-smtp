...
err_code_t serve_conn_event(conn_state_t *restrict conn_state, char *restrict err_msg_buf) {
    ...
    if (pollfd.revents & POLLIN) {
        ...
        if (recv(sock_fd, message_buf, MAX_MESSAGE_LENGTH, 0) < 0) {
            snprintf(err_msg_buf, ERROR_MESSAGE_MAX_LENGTH, "socket_fd=%d", sock_fd);
...
void socket_init(int socket_fd, size_t pos, conn_state_t *restrict conn_pool) {
    ...
    conn_pool[pos].fsm_state = smtp_server_fsm_step(&command);
}

err_code_t accept_conn(conn_pool_t *restrict conn_pool, char *restrict err_msg_buf) {
    assert(conn_pool != NULL && err_msg_buf != NULL);
    ...
    if ((client_socket = accept(conn_pool->pool[0].pollfd.fd, (sockaddr_t *) &client_addr, &addr_len)) < 0) {
        ...
        if (conn_pool->size == conn_pool->capacity) {  // if pool is full
            ...
            if (i < conn_pool->size) { // If there is free pos in allocated pool
                ...
                if ((conn_pool->pool = (conn_state_t *) realloc(
                        conn_pool->pool, sizeof(conn_state_t) * conn_pool->capacity)) == NULL) {
        ...
        socket_init(client_socket, socket_pool_pos, conn_pool->pool);
    }
}
...
err_code_t serve(int server_socket, char *restrict err_msg_buf) {
    ...
    while (true) {
        ...
        if (conn_pool.pool[SERVER_SOCKET_IDX].pollfd.revents) {
            if ((rc = accept_conn(&conn_pool, err_msg_buf)) != OP_SUCCESS) {
                return rc;
            }
            ready_count--;
        }
        ...
        for (int i = 1; i < conn_pool.size && ready_count > 0; i++) {
            if (conn_pool.pool[i].pollfd.revents) {
                ...
                rc = serve_conn_event(conn_state, err_msg_buf);

                if (rc != OP_SUCCESS) {
                    log_error_message(rc, err_msg_buf);
                } else if (conn_state->fsm_state == SMTP_SERVER_FSM_ST_DONE) {
